...
export default {
  ...
  toggle() {
    if (this.modalPanel.isVisible()) {
      document.removeEventListener('mouseover', this.handler.bind(this));
      this.modalPanel.hide();
    } else {
      document.addEventListener('mouseover', this.handler.bind(this));
  ...
  handler(event) {
    ...
    if (event.path != null && event.path.length > 0) {
      ...
      if (stop > 0) {
        ...
        this.view.update(event.screenX, event.screenY, text, scopes);
      }
    }
  }
};
